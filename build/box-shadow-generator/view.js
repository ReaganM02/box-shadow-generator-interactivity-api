...
(() => {
/*!******************************************!*\
  !*** ./src/box-shadow-generator/view.ts ***!
  \******************************************/
...
const colorPicker = (el, color) => {
  return _simonwep_pickr__WEBPACK_IMPORTED_MODULE_1___default().create({
    el: el,
    theme: 'nano',
    default: color,
    components: {
...
const {
  state,
  actions
} = (0,_wordpress_interactivity__WEBPACK_IMPORTED_MODULE_0__.store)('boxShadowGenerator', {
  state: {
    get buildBoxShadow() {
      const context = (0,_wordpress_interactivity__WEBPACK_IMPORTED_MODULE_0__.getContext)();
      return context.shadows.map(shadow => {
        const {
          inset,
          horizontalOffset,
          verticalOffset,
  ...
  callbacks: {
    colorPickerItem() {
      ...
      const context = (0,_wordpress_interactivity__WEBPACK_IMPORTED_MODULE_0__.getContext)();
      if (ref) {
        const boxShadowEl = ref.querySelector('.box-shadow-color-picker');
        if (boxShadowEl) {
          const pickr = colorPicker(boxShadowEl, context.item.data.color);
          pickr.on('change', (HSVaColorObject, eventSource, PickrInstance) => {
